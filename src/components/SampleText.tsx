...
export default function prepopulatedText() {
  ...
  list.append(
    $createListItemNode().append($createTextNode(`These are links (e.g., `), $createLinkNode("https://lexical.dev/").append($createTextNode("Lexical website")), $createTextNode(`).`)),
    $createListItemNode().append($createTextNode(`Press twice on the link to see and edit the URL.`))
  );
  root.append(list);
  const paragraph3 = $createParagraphNode();
  paragraph3.append($createTextNode(`Press the bottom right button to switch to Markdown & vice versa.`));
